...
export function ProductChatbot() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      role: 'assistant',
      content: 'Hello! 👋 I\'m HealthBot, your AI assistant for product hunting in GrabHealth. I can help you with:'
    },
    {
      id: '2',
      role: 'assistant',
      content: '• Finding products based on your requirements\n• Providing information about health products\n• Explaining product benefits and usage\n• Recommending similar products'
    }
  ])
  ...
  const formatBotMessage = (content: string) => {
    ...
    formattedContent = formattedContent
      .replace(/^\s*•\s*(.+)$/gm, '<div class="flex items-start mb-1"><span class="text-emerald-500 mr-2 mt-0.5">•</span><span>$1</span></div>')
      .replace(/^\s*-\s*(.+)$/gm, '<div class="flex items-start mb-1"><span class="text-emerald-500 mr-2 mt-0.5">•</span><span>$1</span></div>')
    
    // Process numbered lists
    formattedContent = formattedContent
      .replace(/^\s*(\d+)\.(\s*.+)$/gm, '<div class="flex items-start mb-1"><span class="text-emerald-600 font-medium mr-2">$1.</span><span>$2</span></div>')
  ...
  return (
    <>
      ...
      >
        <Image src="/robot.png" alt="HealthBot" width={40} height={40} className="h-14 w-14 sm:h-20 sm:w-20" />
      </Button>
      ...
      {isOpen && (
        <div 
          ...
          style={{ 
            animation: 'chatbotAppear 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards',
          ...
          <div className="border-b p-3 sm:p-4 flex items-center justify-between bg-gradient-to-r from-emerald-500 to-teal-600 text-white rounded-t-none sm:rounded-t-2xl">
            <div className="flex items-center gap-3">
              <div className="bg-white bg-opacity-20 p-2.5 rounded-full shadow-inner">
                <Image src="/robot.png" alt="HealthBot" width={28} height={28} className="h-7 w-7" />
              </div>
              <div>
                <h2 className="font-bold text-base sm:text-lg tracking-tight">HealthBot</h2>
                <p className="text-xs opacity-80 font-medium">Your Personal Health Assistant</p>
              </div>
            </div>
            <Button variant="ghost" size="icon" onClick={() => setIsOpen(false)} className="text-white hover:bg-white hover:bg-opacity-20 rounded-full h-9 w-9 transition-colors">
              <X className="h-4 w-4" />
            </Button>
          </div>
                ...
                >
                  {message.role === 'assistant' && (
                    <div className="h-10 w-10 rounded-full bg-emerald-100 flex items-center justify-center mr-2 flex-shrink-0 mt-1">
                      <Image src="/robot.png" alt="HealthBot" width={24} height={24} className="h-6 w-6" />
                    </div>
                  )}
                  <div 
              ...
              {recommendedProducts.length > 0 && (
                <div className="my-4 ml-10">
                  <div className="flex items-center mb-2">
                    ...
                    <h3 className="text-sm font-medium text-emerald-700">Recommended for you</h3>
                  </div>
                  <div className="grid grid-cols-1 gap-2 sm:gap-3 ml-4 sm:ml-8">
                    {recommendedProducts.map((product) => (
                      <Card key={product.id} className="overflow-hidden border-emerald-100 hover:border-emerald-200 transition-colors product-card">
                        <CardContent className="p-4">
                          <div className="flex items-start gap-4">
                            <div className="w-16 sm:w-20 h-16 sm:h-20 relative flex-shrink-0 rounded-lg overflow-hidden border border-emerald-100 shadow-sm group-hover:shadow-md transition-all duration-200">
                              ...
                              />
                              {product.in_stock ? (
                                <div className="absolute bottom-0 left-0 right-0 bg-emerald-500 text-white text-[10px] font-medium py-0.5 text-center">In Stock</div>
                              ) : (
                                <div className="absolute bottom-0 left-0 right-0 bg-gray-500 text-white text-[10px] font-medium py-0.5 text-center">Out of Stock</div>
                              )}
                            </div>
                            <div className="flex-1 min-w-0">
                              <h4 className="font-medium text-sm truncate text-gray-800">{product.name}</h4>
                              <p className="text-xs text-gray-500 mt-1 line-clamp-2">{product.description}</p>
                              <div className="flex items-center justify-between mt-3">
                                <div>
                                  <p className="font-semibold text-emerald-600">{formatPrice(product.price)}</p>
                                  {product.discount_premium > 0 && (
                                    <p className="text-xs text-gray-500 line-through">{formatPrice(product.price / (1 - product.discount_premium))}</p>
                                  )}
                                </div>
                                <Button 
                                  size="sm" 
