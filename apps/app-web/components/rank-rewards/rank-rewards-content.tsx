...
import { Award, Gift, Star, TrendingUp, Users, Zap, ChevronRight } from "lucide-react"
        ...
        <Tabs defaultValue="ranks" className="w-full">
          <div className="px-6 pt-6 pb-4">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div>
                <h2 className="text-xl font-semibold text-gray-800 dark:text-gray-100">Membership Program</h2>
                <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">Track your progress and earn exclusive rewards</p>
              </div>
    ...
    <div className="h-full bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 overflow-hidden">
      <div className="p-4 border-b border-gray-200 dark:border-gray-800">
        <div className="flex items-center justify-between">
          ...
          <Badge className="bg-yellow-500 hover:bg-yellow-600 text-white text-xs font-medium">{currentRank}</Badge>
        </div>
      </div>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-2">
          <div className="flex justify-between text-xs">
            <span className="font-medium text-gray-700 dark:text-gray-300">Progress to {nextRank}</span>
            <span className="font-medium text-gray-700 dark:text-gray-300">{totalPoints} / {requiredPoints}</span>
          </div>
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400">
            <span className="font-medium text-yellow-600 dark:text-yellow-400">{pointsToNextRank}</span> more points to reach {nextRank}
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-gray-50 dark:bg-gray-800 p-3 rounded-lg border border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-750 transition-colors">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-xs font-medium text-gray-700 dark:text-gray-300">Total Points</span>
            </div>
            <p className="text-xl font-bold mt-1 text-yellow-600 dark:text-yellow-400">{totalPoints}</p>
          </div>
          
          <div className="bg-gray-50 dark:bg-gray-800 p-3 rounded-lg border border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-750 transition-colors">
            <div className="flex items-center space-x-2">
              <Users className="h-3.5 w-3.5 text-blue-500" />
              <span className="text-xs font-medium text-gray-700 dark:text-gray-300">Network Size</span>
            </div>
            <p className="text-xl font-bold mt-1 text-blue-600 dark:text-blue-400">7</p>
          </div>
        </div>
      </div>
    </div>
...
function RankBenefits() {
  ...
  const filteredRanks = selectedView === "all" 
    ? ranks 
    ...
    <div className="h-full bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 overflow-hidden">
      <div className="p-4 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-500">Unlock more benefits as you rank up</p>
        </div>
                ...
                <div className="p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <div className={`p-2 rounded-full ${colorClass.bg} ${colorClass.darkBg}`}>
                  ...
                  <div className="space-y-2 mt-2">
                    ...
                    {rank.benefits.length > 4 && (
                      <Button variant="link" size="sm" className="text-xs h-6 px-0 py-0 text-blue-600 dark:text-blue-400">
                  ...
                  {isCurrentRank && (
                    <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
                      <Badge className={`text-xs bg-yellow-100 text-yellow-800 border border-yellow-200 dark:bg-yellow-900/30 dark:text-yellow-400 dark:border-yellow-800/50`}>
                        Current Rank
                      </Badge>
                    </div>
                  )}
        ...
        <CardHeader className="pb-3">
          <div className="flex justify-between items-center">
            <div>
              ...
              <CardDescription className="text-xs text-gray-500">Your points and activity</CardDescription>
            </div>
        ...
        <CardContent className="pt-0">
          <div className="grid grid-cols-3 gap-3 mb-4">
            <div className="bg-gradient-to-br from-yellow-50 to-amber-100 dark:from-yellow-950/40 dark:to-amber-900/20 p-3 rounded-lg border border-yellow-200 dark:border-yellow-900/50">
              <div className="flex items-center space-x-1.5">
                ...
                <span className="text-xs font-medium text-yellow-700 dark:text-yellow-400">Available</span>
              </div>
              <p className="text-xl font-bold mt-1 text-yellow-700 dark:text-yellow-400">{availablePoints}</p>
            </div>
            ...
            <div className="bg-muted/50 p-3 rounded-lg border border-border/50">
              ...
              <p className="text-xl font-bold mt-1 text-[#0C99B4] dark:text-[#35b4ca]">{totalEarned}</p>
            </div>
            ...
            <div className="bg-muted/50 p-3 rounded-lg border border-border/50">
              ...
              <p className="text-xl font-bold mt-1 text-purple-600 dark:text-purple-400">{rewardsRedeemed}</p>
            </div>
          </div>
          ...
          <div className="space-y-1">
            ...
            <div className="space-y-1 max-h-[180px] overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-muted scrollbar-track-transparent">
              {[
                { description: "Commission from order #1234", points: 150, date: "May 12, 2025" },
                { description: "Rank upgrade bonus", points: 100, date: "May 5, 2025" },
                { description: "Referral bonus - New member", points: 50, date: "Apr 28, 2025" },
                { description: "Commission from order #1187", points: 75, date: "Apr 15, 2025" }
              ].map((activity, index) => (
                <div key={index} className="flex items-center justify-between py-1.5 border-b border-border/40 last:border-0">
                  <div>
                    <p className="text-xs font-medium">{activity.description}</p>
                    <p className="text-xs text-muted-foreground">{activity.date}</p>
                  </div>
                  <Badge variant="outline" className="text-xs text-[#0C99B4] h-5 px-1.5">+{activity.points}</Badge>
                </div>
              ))}
            </div>
          </div>
        ...
        <CardHeader className="pb-3">
          <div className="flex justify-between items-center">
            <div>
              ...
              <CardDescription className="text-xs text-gray-500">Redeem your points for rewards</CardDescription>
            </div>
            <Badge variant="secondary" className="text-sm">
              <Star className="mr-1.5 h-3.5 w-3.5 text-yellow-500" /> {availablePoints} points
            </Badge>
          </div>
        </CardHeader>
        <CardContent className="pt-0">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {availableRewards.map((reward, index) => (
              <div key={index} className="border rounded-lg p-3 hover:border-primary/50 hover:bg-muted/50 transition-colors cursor-pointer group">
                <div className="flex justify-between items-start mb-2">
                  <div className="flex items-center gap-2">
                    <div className="text-2xl">{reward.image}</div>
                    <div>
                      <h4 className="text-sm font-medium group-hover:text-primary transition-colors">{reward.name}</h4>
                ...
                <div className="w-full bg-muted h-1.5 rounded-full overflow-hidden">
                  <div 
                    ...
                    style={{ width: `${Math.min(100, (availablePoints / reward.points) * 100)}%` }}
                  ></div>
                </div>
                <p className="text-xs mt-1 text-right">
                  {availablePoints >= reward.points 
                    ? <span className="text-[#0C99B4] dark:text-[#35b4ca]">Available to redeem</span>
                    : <span className="text-muted-foreground">Need {reward.points - availablePoints} more points</span>
                  }
                </p>
              </div>
            ))}
