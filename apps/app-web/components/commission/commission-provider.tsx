...
export function CommissionProvider({ children }: { children: React.ReactNode }) {
  ...
  const initializeCommissionSystem = async () => {
    try {
      ...
      if (!initResponse.ok) {
        console.error("Failed to initialize commission system:", await initResponse.text())
      } else {
  ...
  const fetchCommissionData = async () => {
    ...
    try {
      ...
      try {
        ...
        console.error("Error initializing commission system, but continuing:", initError)
      ...
      try {
        ...
        if (!response.ok) {
          console.warn("Commission API returned non-OK status:", response.status)
          // Set default values but don't throw error to allow UI to render
          setUpline(null)
          setDownlines([])
          setCommissions([])
          setPoints(0)
          setReferralLink(`${window.location.origin}/auth/register?referrer=${user.id || 'user'}`)
          setTotalEarnings(0)
          return
        }
        ...
        setReferralLink(data.referralLink || `${window.location.origin}/auth/register?referrer=${user.id || 'user'}`)
        
        // Calculate total earnings from commissions
        const total = (data.commissions || []).reduce((sum: number, commission: Commission) => {
