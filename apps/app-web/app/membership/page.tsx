...
import { Loader2, Check, X, ArrowRight, ArrowUpRight, ChevronRight, Star, Shield, Truck, BadgePercent } from "lucide-react"
import Image from "next/image"
import Link from "next/link"

const MembershipLevels = dynamic(() => import("@/components/MembershipLevels"), {
  ssr: false,
  loading: () => <div className="h-[600px] bg-gray-100 rounded-lg animate-pulse"></div>,
})
    ...
    try {
      if (tierName === 'level7') {
        ...
        if (membership.points < requiredPoints) {
          toast.error(`You need ${requiredPoints} points to upgrade to ${nextTier}`);
          setIsUpgrading(false);
          return;
        }
      ...
      toast.error(error instanceof Error ? error.message : 'Failed to process membership action')
    } finally {
      setIsJoining(false)
      setIsUpgrading(false)
    }
    ...
    <div className="container mx-auto px-4 py-6 md:px-6">
      <div className="text-center mb-4">
        ...
        <p className="text-gray-600 text-sm max-w-2xl mx-auto">
          Unlock exclusive health benefits and discounts on products and services
        </p>
      </div>

      <div className="max-w-5xl mx-auto">
        <h2 className="text-lg font-bold mb-3 text-center">Membership Benefits</h2>
