...
const sampleCategories = [
  ...
  {
    name: 'Supplements',
    description: 'Health supplements for specific needs',
    icon: 'flask',
  },
  {
    name: 'First Aid',
    description: 'First aid supplies for emergencies',
    icon: 'bandage',
  },
  {
    name: 'Personal Care',
...
async function initializeDatabase() {
  try {
    ...
    const existingCategories =
      await sql`SELECT COUNT(*) as count FROM product_categories`;
    const categoryCount = parseInt(
      existingCategories[0]?.count?.toString() || '0'
    );
    ...
    const productCount = parseInt(
      existingProducts[0]?.count?.toString() || '0'
    );
  ...
  try {
    ...
    try {
      ...
      if (search) {
        ...
        conditions.push(
          sql`(p.name ILIKE ${searchPattern} OR p.description ILIKE ${searchPattern})`
        );
      }
      ...
      if (conditions.length > 0) {
        ...
        const whereConditions = conditions.reduce(
