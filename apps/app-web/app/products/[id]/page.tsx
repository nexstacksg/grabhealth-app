...
export default function ProductDetailPage() {
  ...
  const tierDiscount = useMemo(() => {
    if (!product) return 0;
    if (membership?.tier && ["level4", "level5", "level6", "level7"].includes(membership.tier)) {
      return Math.round((typeof product.discount_premium === 'string' ? parseFloat(product.discount_premium) : product.discount_premium) * 100);
    } else if (membership?.tier && ["level1", "level2", "level3"].includes(membership.tier)) {
      return Math.round((typeof product.discount_essential === 'string' ? parseFloat(product.discount_essential) : product.discount_essential) * 100);
    }
    return 0;
  }, [product, membership]);
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        const response = await fetch(`/api/products/${params.id}?t=${Date.now()}`)
        ...
        if (!response.ok) {
          throw new Error(`Failed to fetch product: ${response.status} ${responseText}`)
        }
  ...
  const addDemoData = (product: Product): Product => {
    ...
    if (!product.features || product.features.length === 0) {
      ...
      product.features = demoFeatures[product.category as keyof typeof demoFeatures] || [
    ...
    if (!product.usage) {
      const demoUsage: Record<string, string> = {
        "Pain Relief": "Adults and children 12 years and over: Take 1-2 tablets every 4-6 hours as needed, not exceeding 6 tablets in 24 hours. Take with food or milk if stomach upset occurs. Consult a doctor for use in children under 12 years of age.",
        "Cold & Flu": "Adults and children 12 years and over: Take 2 capsules every 6 hours, not exceeding 8 capsules in 24 hours. Do not use for more than 7 days unless directed by a doctor. Not recommended for children under 12 years.",
        "Vitamins": "Adults: Take one tablet daily with a meal. For best absorption, take with a source of fat. Do not exceed recommended dosage. Store in a cool, dry place away from direct sunlight.",
        "First Aid": "Clean the affected area thoroughly with mild soap and water before application. Apply a thin layer to the affected area 1-3 times daily as needed. Cover with a sterile bandage if necessary. Consult a healthcare professional for deep or puncture wounds."
      };
      
      product.usage = demoUsage[product.category as keyof typeof demoUsage] || "Take as directed on the package or as advised by your healthcare professional. Store in a cool, dry place away from direct sunlight and out of reach of children. Do not use if safety seal is broken or missing.";
    }
    
    // Add demo ingredients if not present
    if (!product.ingredients) {
      const demoIngredients: Record<string, string> = {
        "Pain Relief": "Active Ingredients: Acetaminophen 500mg, Ibuprofen 200mg. Inactive Ingredients: Microcrystalline cellulose, corn starch, sodium starch glycolate, colloidal silicon dioxide, magnesium stearate, hypromellose, polyethylene glycol.",
        "Cold & Flu": "Active Ingredients: Acetaminophen 325mg, Dextromethorphan HBr 10mg, Phenylephrine HCl 5mg. Inactive Ingredients: Gelatin, corn starch, silicon dioxide, titanium dioxide, FD&C Blue No. 1, FD&C Red No. 40, vitamin C (ascorbic acid) 60mg.",
        "Vitamins": "Vitamin A (as retinyl acetate) 5000 IU, Vitamin C (as ascorbic acid) 60mg, Vitamin D (as cholecalciferol) 400 IU, Vitamin E (as dl-alpha tocopheryl acetate) 30 IU, Vitamin K (as phytonadione) 25mcg, Thiamin (as thiamin mononitrate) 1.5mg, Riboflavin 1.7mg, Niacin (as niacinamide) 20mg, Vitamin B6 (as pyridoxine HCl) 2mg, Folate (as folic acid) 400mcg, Vitamin B12 (as cyanocobalamin) 6mcg, Biotin 30mcg, Pantothenic Acid (as d-calcium pantothenate) 10mg, Calcium (as calcium carbonate) 200mg, Iron (as ferrous fumarate) 18mg, Phosphorus (as dicalcium phosphate) 20mg, Iodine (as potassium iodide) 150mcg, Magnesium (as magnesium oxide) 100mg, Zinc (as zinc oxide) 15mg, Selenium (as sodium selenate) 20mcg, Copper (as copper sulfate) 2mg, Manganese (as manganese sulfate) 2mg, Chromium (as chromium picolinate) 120mcg, Molybdenum (as sodium molybdate) 75mcg, Chloride (as potassium chloride) 72mg, Potassium (as potassium chloride) 80mg, Boron (as sodium borate) 150mcg, Nickel (as nickel sulfate) 5mcg, Silicon (as silicon dioxide) 2mg, Tin (as stannous chloride) 10mcg, Vanadium (as sodium metavanadate) 10mcg.",
        "First Aid": "Active Ingredient: Benzalkonium Chloride 0.13%. Inactive Ingredients: Purified water, glycerin, hydroxypropyl methylcellulose, propylene glycol, aloe vera extract, lavender oil, tea tree oil, vitamin E acetate."
      };
      
      product.ingredients = demoIngredients[product.category as keyof typeof demoIngredients] || "Active and inactive ingredients listed on packaging. Please refer to the product label for complete ingredient information. Contains no artificial colors, flavors, or preservatives unless otherwise stated.";
    }
  ...
  const calculateDiscountedPrice = () => {
    ...
    const discountPremium = typeof product.discount_premium === 'string' ? parseFloat(product.discount_premium) : product.discount_premium
    const discountEssential = typeof product.discount_essential === 'string' ? parseFloat(product.discount_essential) : product.discount_essential
    ...
    return (
      <div className="container max-w-6xl py-20 flex items-center justify-center min-h-[60vh]">
        <div className="text-center">
          ...
          <p className="text-gray-500 mb-6">{error || "This product could not be found or may have been removed."}</p>
          <Link href="/products">
            <Button>
  ...
  return (
    <div className="container max-w-6xl py-6 md:py-10 px-4 md:px-6">
      {/* Breadcrumb */}
      <div className="mb-6">
        <Link href="/products" className="text-sm text-gray-500 hover:text-emerald-600 flex items-center group transition-all duration-200">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-10 mb-10 md:mb-16">
        ...
        <div className="relative rounded-xl overflow-hidden bg-white border border-gray-100 h-[350px] md:h-[450px] group">
          ...
          {/* Discount Badge */}
          {membership?.tier && ["level4", "level5", "level6", "level7"].includes(membership.tier) && product.discount_premium > 0 && (
            <Badge className="absolute bottom-4 left-4 bg-red-500 hover:bg-red-600 transition-all duration-200 px-3">
              <div className="flex items-center">
                <span className="mr-1">🔥</span>
                {Math.round((typeof product.discount_premium === 'string' ? parseFloat(product.discount_premium) : product.discount_premium) * 100)}% OFF
              </div>
            </Badge>
          )}
          {membership?.tier && ["level1", "level2", "level3"].includes(membership.tier) && product.discount_essential > 0 && (
            <Badge className="absolute bottom-4 left-4 bg-red-500 hover:bg-red-600 transition-all duration-200 px-3">
              <div className="flex items-center">
                <span className="mr-1">🔥</span>
                {Math.round((typeof product.discount_essential === 'string' ? parseFloat(product.discount_essential) : product.discount_essential) * 100)}% OFF
              </div>
            </Badge>
          )}
        </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-4 md:p-6 mb-4 md:mb-6">
            ...
            <div className="flex flex-wrap items-center gap-3 mb-5">
              <div className="flex items-center bg-yellow-50 px-3 py-1 rounded-full">
                {[...Array(5)].map((_, i) => (
                  <Star key={i} className={`h-4 w-4 ${i < 4 ? "text-yellow-500" : "text-gray-300"}`} fill="currentColor" />
                ))}
                <span className="text-gray-700 ml-2 text-sm font-medium">4.0</span>
              </div>
            ...
            <div className="mb-4 md:mb-6 bg-gray-50 p-3 md:p-4 rounded-lg">
              <div className="flex flex-wrap items-baseline gap-3 mb-2">
                <span className="text-2xl md:text-3xl font-bold text-emerald-600">{formatPrice(calculateDiscountedPrice())}</span>
                {calculateDiscountedPrice() < product.price && (
                  <span className="text-gray-500 line-through text-base md:text-lg">{formatPrice(product.price)}</span>
                )}
              </div>
              ...
              {membership && tierDiscount > 0 && (
                <div className="mt-2 bg-emerald-50 px-2 md:px-3 py-1.5 rounded-lg inline-flex items-center max-w-full overflow-hidden">
                  <span className="text-xs md:text-sm text-emerald-700 font-medium truncate">
                    {tierDiscount}% discount with your {membership.tier.replace('level', 'Level ')} membership
                  </span>
                </div>
              )}
            </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
            <Tabs defaultValue="features" className="w-full">
              <TabsList className="w-full grid grid-cols-3 bg-gray-50 p-0 h-auto">
                <TabsTrigger value="features" className="py-3 text-sm md:text-base data-[state=active]:bg-white">
                  Features
                </TabsTrigger>
                <TabsTrigger value="usage" className="py-3 text-sm md:text-base data-[state=active]:bg-white">
                  Usage
                </TabsTrigger>
                <TabsTrigger value="ingredients" className="py-3 text-sm md:text-base data-[state=active]:bg-white">
                  Ingredients
                </TabsTrigger>
              </TabsList>
              ...
              <TabsContent value="usage" className="p-4 md:p-6">
                {product.usage ? (
                  ...
                  <p className="text-gray-500 italic">No usage information available for this product.</p>
                )}
              </TabsContent>
                  ...
                  <div>
                    ...
                    <div className="bg-yellow-50 p-3 rounded-lg border border-yellow-100">
                      <p className="text-sm text-yellow-800">
                        <span className="font-medium">Note:</span> If you have allergies or specific health concerns, please consult with a healthcare professional before use.
                      </p>
                    </div>
                  </div>
                ) : (
                  <p className="text-gray-500 italic">No ingredients information available for this product.</p>
                )}
              </TabsContent>
            </Tabs>
          </div>
              ...
              <Link href={`/products/${relatedProduct.id}`} key={relatedProduct.id}>
                <Card className="h-full hover:shadow-md transition-shadow overflow-hidden">
                  ...
                  <CardContent className="p-4">
                    ...
                    <p className="text-sm text-gray-500 mb-2 line-clamp-2">{relatedProduct.description}</p>
                    <div className="flex items-center justify-between">
                      <span className="font-medium text-emerald-600">{formatPrice(relatedProduct.price)}</span>
                      {!relatedProduct.in_stock && (
                        <Badge variant="outline" className="text-xs text-gray-500">Out of Stock</Badge>
                      )}
                    </div>
                  </CardContent>
                </Card>
