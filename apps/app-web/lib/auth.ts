  ...
  try {
    ...
    if (Array.isArray(tableColumns) && tableColumns.length > 0) {
      const columns = tableColumns.map(col => {
        ...
        return col && typeof col === 'object' && 'column_name' in col ? col.column_name : 'unknown';
      }).join(', ');
...
export async function createUser(name: string, email: string, password: string): Promise<User> {
  try {
    ...
    console.log(`Creating user with name: ${name}, email: ${email}, password hash and salt are set`);
    ...
    try {
      ...
      const tableInfo = await sql`SELECT * FROM information_schema.columns WHERE table_name = 'users'`;
...
export async function setAuthCookie(userId: number) {
  ...
  const sessionValue = JSON.stringify({ userId, expires: Date.now() + 7 * 24 * 60 * 60 * 1000 })
...
export async function getCurrentUser() {
  ...
  try {
    ...
    const users = await sql`SELECT id, name, email, image_url, role FROM users WHERE id = ${session.userId}`
