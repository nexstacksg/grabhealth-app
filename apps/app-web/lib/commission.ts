...
export async function getUserUpline(
  userId: number
...
export async function getUserDownlines(
  userId: number
...
export async function setUserUpline(
  userId: number,
  uplineId: number,
  level: number = 1
): Promise<UserRelationship> {
  try {
    ...
    if (
      Array.isArray(existingRelationship) &&
      existingRelationship.length > 0
    ) {
...
export async function calculateCommissions(
  ...
  try {
    ...
    const tiers =
      (await sql`SELECT * FROM commission_tiers ORDER BY tier_level ASC`) as CommissionTier[];
    ...
    for (const upline of uplineChain) {
      ...
      if (level === 1) {
        ...
        if (tier.points_rate > 0) {
          await addUserPoints(
            upline.upline_id!,
            Math.floor((orderTotal * tier.points_rate) / 100)
          );
        }
      }
