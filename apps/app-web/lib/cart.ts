...
export async function addToCart(
  ...
  try {
    ...
    if (Array.isArray(existingItems) && existingItems.length > 0) {
      ...
      return Array.isArray(updatedItem) ? updatedItem[0] as CartItem : {} as CartItem
    } else {
      // Add new item
      ...
      return Array.isArray(newItem) ? newItem[0] as CartItem : {} as CartItem
    }
  } catch (error) {
...
export async function convertCartToOrder(userId: number): Promise<number | null> {
  try {
    // Get cart items
    ...
    try {
      ...
      console.log(`Calculated ${commissions.length} commissions for order ${orderId}`)
