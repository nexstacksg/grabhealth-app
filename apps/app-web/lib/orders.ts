...
export async function initializeOrdersTables() {
  try {
    ...
    if (Array.isArray(tablesCheck)) {
      const tableNames = tablesCheck.map(t => {
        return typeof t === 'object' && t !== null && 'table_name' in t ? t.table_name : 'unknown';
      }).join(', ');
    ...
    return orders.map(order => {
      ...
      if (typeof order === 'object' && order !== null) {
        ...
        return {
          ...
          total: typeof total === 'string' ? parseFloat(total) : Number(total || 0)
        };
      }
      return order;
    }) as Order[]
...
export async function getOrderById(orderId: number, userId: number): Promise<Order | null> {
  try {
    ...
    const order: Order = {
      ...
      total: typeof orderObj.total === 'string' ? parseFloat(orderObj.total) : Number(orderObj.total || 0),
      ...
      order.items = items.map(item => {
        if (typeof item === 'object' && item !== null) {
          ...
          return {
            ...
            price: typeof itemObj.price === 'string' ? parseFloat(itemObj.price) : Number(itemObj.price || 0),
...
export async function addSampleOrders(userId: number) {
  try {
    ...
    const existingOrders = await sql`SELECT COUNT(*) FROM orders WHERE user_id = ${userId}`
    
    if (Array.isArray(existingOrders) && existingOrders.length > 0) {
      const firstResult = existingOrders[0];
      const count = typeof firstResult === 'object' && firstResult !== null && 'count' in firstResult
    ...
    const products = [
      ...
      { id: 2, name: "Digital Thermometer", price: 24.50 },
