...
async function main() {
  ...
  const premiumTier = await prisma.membershipTier.upsert({
    ...
    create: {
      ...
      discount: 0.25,
      benefits:
        'All Essential benefits, Priority customer support, Free shipping on orders over $50, Quarterly gift items',
    },
  });
  ...
  const commissionTiers = [
    {
      tierLevel: 1,
      tierName: 'Tier 1 (Direct Sales)',
      directCommissionRate: 0.3,
      indirectCommissionRate: 0.0,
      pointsRate: 0,
    },
    {
      tierLevel: 2,
      tierName: 'Tier 2 (Indirect Sales)',
      directCommissionRate: 0.3,
