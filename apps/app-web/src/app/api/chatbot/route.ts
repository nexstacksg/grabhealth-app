...
async function getRelevantProducts(message: string) {
  try {
    ...
    const categoryKeywords = keywords.filter((keyword) =>
      ['Vitamins', 'Supplements', 'First Aid', 'Personal Care'].includes(
        keyword
      )
    );
...
Do not make up any products that aren't in the list provided. Avoid overly formal language or medical jargon.`;
    ...
    const recommendedProducts = determineRecommendedProducts(
      aiMessage,
      products
    );
    ...
    return {
      message:
        "Sorry about that! I'm having a bit of trouble connecting to our product database right now. Could you try asking me something else about health products, or try again in a moment?",
...
function determineRecommendedProducts(aiMessage: string, products: any[]) {
  ...
  const productScores = products.map((product) => {
    const nameMatches = (aiMessage.match(new RegExp(product.name, 'gi')) || [])
      .length;
    const categoryMatches = (
      aiMessage.match(new RegExp(product.category, 'gi')) || []
    ).length;
    ...
    const descriptionMatches = descriptionTerms.reduce(
