...
export async function PATCH(request: Request) {
  try {
    ...
    console.log(
      `Attempting to update membership for user ${user.id} with points=${points}, tier=${tier}`
    );
    ...
    if (!Array.isArray(existingMembership) || existingMembership.length === 0) {
      ...
      console.log(
        `No existing membership found for user ${user.id}, creating new one`
      );
      const newMembership = await sql`
        INSERT INTO memberships (user_id, tier, points)
      ...
      return NextResponse.json(
        Array.isArray(newMembership) ? newMembership[0] : {}
      );
    }
