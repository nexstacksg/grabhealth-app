  ...
  try {
    ...
    try {
      const relationshipsCount =
        await sql`SELECT COUNT(*) FROM user_relationships`;
      hasRelationships =
        relationshipsCount &&
        Array.isArray(relationshipsCount) &&
        relationshipsCount.length > 0 &&
        relationshipsCount[0].count > 0;
    } catch (error) {
...
async function createSampleUsers() {
  try {
    ...
    for (const user of sampleUsers) {
      ...
      const existingUser =
        await sql`SELECT id, name, email FROM users WHERE email = ${user.email}`;
    ...
    const currentUser =
      await sql`SELECT id, name, email FROM users ORDER BY id DESC LIMIT 1`;
...
async function createSampleCommissions(orders: any[]) {
  try {
    ...
    try {
      ...
      hasCommissions =
        commissionsCount &&
        Array.isArray(commissionsCount) &&
        commissionsCount.length > 0 &&
        commissionsCount[0].count > 0;
    } catch (error) {
    ...
    for (const order of orders) {
      try {
        ...
        console.error(
          `Error calculating commission for order ${order?.id}:`,
          error
        );
        // Continue with next order
        continue;
