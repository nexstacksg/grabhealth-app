    ...
    if (Array.isArray(tiersCount) && tiersCount.length > 0) {
      ...
      if (count === '0' || count === 0) {
        ...
        if (Array.isArray(products)) {
          ...
          const ginSengWater = products.find((p) =>
            p.name.includes('Golden GinSeng Water')
          );
          if (ginSengWater) {
            await sql`
          ...
          // Honey Wild GinSeng
          const honeyGinSeng = products.find((p) =>
            p.name.includes('Honey Wild GinSeng')
          );
          if (honeyGinSeng) {
            await sql`
              INSERT INTO product_commission_tiers (
...
export async function getProductCommissionTiers(): Promise<
  ProductCommissionTier[]
> {
  try {
...
export async function getVolumeBonus(
  salesVolume: number
): Promise<VolumeBonus | null> {
  try {
...
export async function calculateProductCommission(
  ...
  try {
    ...
    if (volumeBonus && volumeBonus.bonus_percentage > 0) {
      ...
      const bonusAdjustedRate =
        minCommissionRate + volumeBonus.bonus_percentage / 100;
      finalCommissionRate = Math.min(bonusAdjustedRate, maxCommissionRate);
    }

    // Calculate commission amount based on retail price
    const commissionAmount =
      tier.retail_price * quantity * (finalCommissionRate / 100);
